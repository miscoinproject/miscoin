...
const string strMessageMagic = "MIScoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    int blockNumber = pindexBest->nHeight;
    
    if(blockNumber > LAST_POW_BLOCK) {
        return 0;
    }
    ...
    else if (blockNumber <= 4320) {
      nSubsidy = 100 * COIN;
    }
    else if (blockNumber <= 5760) {
      nSubsidy = 150 * COIN;
    }
    else if (blockNumber <= 8640) {
      nSubsidy = 350 * COIN;
    }
    else if (blockNumber <= 11520) {
      nSubsidy = 450 * COIN;
    }
    else if (blockNumber <= 14400) {
      nSubsidy = 350 * COIN;
    }
    else if (blockNumber <= 15840) {
      nSubsidy = 250 * COIN;
    }
    else if (blockNumber <= 20160) {
      nSubsidy = 100 * COIN;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MIScoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Miscoin 30/01/15";
        CTransaction txNew;
        txNew.nTime = 1422576000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1422576000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2783047;
		if(fTestNet)
        {
            block.nNonce   = 2783047;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x9350963e91e02b5297217d5ca5f2dffbd58003f7947a99cad8bb47129539cf4e"));
        block.print();
